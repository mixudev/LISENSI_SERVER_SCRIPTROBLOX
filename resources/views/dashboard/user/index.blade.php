...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
    ...
    <div class="p-5 space-y-6">
        ...
        <div>
            ...
            <div class="mb-4">
                <div class="flex items-center gap-2 mb-2">
                    <span class="text-[9px] font-bold text-white bg-emerald-600 px-2 py-0.5">METODE 1 — DIREKOMENDASIKAN</span>
                    <p class="text-xs text-slate-600 dark:text-slate-400">Langsung — 1 baris, tidak perlu Loader.lua</p>
                </div>
                <div class="relative bg-slate-900 dark:bg-slate-950 border border-emerald-800/40">
                    <div class="flex items-center justify-between px-4 py-2 border-b border-slate-700 dark:border-slate-800">
                        <span class="text-[10px] font-mono text-emerald-400 uppercase tracking-widest">GET Direct — game:HttpGet()</span>
                        <button onclick="copyCode('code-direct', this)"
                    ...
                    <pre id="code-direct" class="p-4 text-xs font-mono text-slate-300 overflow-x-auto leading-relaxed"><span class="text-slate-500">-- Ganti YOUR_LICENSE_KEY dengan key milikmu</span>
<span class="text-sky-400">loadstring</span>(<span class="text-sky-400">game</span>:<span class="text-amber-300">HttpGet</span>(<span class="text-green-400">"{{ rtrim(config('app.url'), '/') }}/api/license/get?key=YOUR_LICENSE_KEY&hwid=" .. tostring(game:GetService("Players").LocalPlayer.UserId) .. "&username=" .. game:GetService("Players").LocalPlayer.Name .. "&place=" .. tostring(game.PlaceId)</span>))()</pre>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center gap-2 mb-2">
                    <span class="text-[9px] font-bold text-white bg-slate-600 px-2 py-0.5">METODE 2</span>
                    <p class="text-xs text-slate-600 dark:text-slate-400">Via Loader.lua — kompatibilitas format lama</p>
                </div>
                <div class="relative bg-slate-900 dark:bg-slate-950 border border-slate-700 dark:border-slate-800">
                    <div class="flex items-center justify-between px-4 py-2 border-b border-slate-700 dark:border-slate-800">
                        <span class="text-[10px] font-mono text-slate-400 uppercase tracking-widest">Via Loader.lua</span>
                    ...
                    <pre id="code-executor" class="p-4 text-xs font-mono text-slate-300 overflow-x-auto leading-relaxed"><span class="text-slate-500">-- Set key dulu, baru jalankan loader</span>
<span class="text-sky-400">getgenv</span>().<span class="text-sky-400">script_key</span> = <span class="text-green-400">"YOUR_LICENSE_KEY"</span>
<span class="text-sky-400">loadstring</span>(<span class="text-sky-400">game</span>:<span class="text-amber-300">HttpGet</span>(<span class="text-green-400">"{{ rtrim(config('app.url'), '/') }}/Loader.lua"</span>))()</pre>
        ...
        <div>
            ...
            <div class="relative bg-slate-900 dark:bg-slate-950 border border-emerald-800/40 dark:border-emerald-700/30">
                ...
                <pre id="code-ready" class="p-4 text-xs font-mono text-slate-300 overflow-x-auto leading-relaxed"><span class="text-sky-400">loadstring</span>(<span class="text-sky-400">game</span>:<span class="text-amber-300">HttpGet</span>(<span class="text-green-400">"{{ rtrim(config('app.url'), '/') }}/api/license/get?key={{ $firstActive->license_key }}&hwid=" .. tostring(game:GetService("Players").LocalPlayer.UserId) .. "&username=" .. game:GetService("Players").LocalPlayer.Name .. "&place=" .. tostring(game.PlaceId)</span>))()</pre>
            </div>
        </div>
        @endif
