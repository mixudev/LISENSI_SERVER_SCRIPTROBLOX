        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                @php
                    ...
                    $hasDetail = $log->response_message || $log->error_detail;
                    $rowId     = 'row-detail-'.$log->id;
                @endphp
                ...
                @if ($hasDetail)
                <tr id="{{ $rowId }}" class="hidden bg-slate-50 dark:bg-slate-800/40">
                    <td colspan="10" class="px-5 py-4">
                        <div class="space-y-3">
                            @if ($log->response_message)
                            <div>
                                <p class="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-1">Pesan Error</p>
                                <p class="text-xs text-red-600 dark:text-red-400 font-mono bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/40 px-3 py-2">
                            ...
                            @if ($log->error_detail)
                            <div>
                                <p class="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-1">Stack Trace</p>
                                <pre class="text-[10px] font-mono text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-2 overflow-x-auto max-h-40">{{ $log->error_detail }}</pre>
                            </div>
                            @endif
                            <div class="flex items-center gap-4 text-[9px] text-slate-400 font-mono">
                                <span>User-Agent: {{ $log->user_agent ?? '—' }}</span>
                                <span>HWID: {{ $log->hwid_used ?? '—' }}</span>
                            </div>
                        </div>
                    </td>
                </tr>
...
<div class="mt-4 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
    ...
    <div class="flex flex-wrap gap-3">
        @foreach ([
            ['step' => 'done',          'color' => 'emerald', 'desc' => 'Inject berhasil penuh — key valid, HWID ok, script dikirim'],
            ['step' => 'license_check', 'color' => 'red',     'desc' => 'Gagal di tahap validasi lisensi (key tidak valid, banned, expired, HWID mismatch)'],
            ['step' => 'token_created', 'color' => 'blue',    'desc' => 'Token script dibuat (validasi berhasil, menunggu executor download)'],
            ['step' => 'script_served', 'color' => 'violet',  'desc' => 'Script berhasil didownload executor via token'],
        ] as $item)
