...
window.ProductForm = {
    ...
    initForm(formId, source) {
        ...
        ProductForm.bindAvailabilityChecks(formId);
    },

    bindAvailabilityChecks(formId) {
        var root = document.querySelector('[data-product-form="' + formId + '"]');
        if (!root || root.dataset.availabilityBound === '1') {
            return;
        }

        root.dataset.availabilityBound = '1';

        var nameInput = root.querySelector('[data-product-name]');
        var placeInput = root.querySelector('[data-place-ids]');

        var trigger = function () {
            ProductForm.scheduleAvailabilityCheck(formId);
        };

        if (nameInput) {
            nameInput.addEventListener('input', trigger);
        }
        if (placeInput) {
            placeInput.addEventListener('input', trigger);
        }

        ProductForm.scheduleAvailabilityCheck(formId);
    },

    scheduleAvailabilityCheck(formId) {
        clearTimeout(ProductForm._availabilityTimers[formId]);
        ProductForm._availabilityTimers[formId] = setTimeout(function () {
            ProductForm.checkAvailability(formId);
        }, 350);
    },

    checkAvailability(formId) {
        var root = document.querySelector('[data-product-form="' + formId + '"]');
        if (!root) {
            return;
        }

        var slugHint = root.querySelector('[data-slug-hint]');
        var placeHint = root.querySelector('[data-place-hint]');
        var nameInput = root.querySelector('[data-product-name]');
        var placeInput = root.querySelector('[data-place-ids]');
        var currentSlug = root.dataset.currentSlug || '';

        if (currentSlug && slugHint) {
            slugHint.textContent = 'Slug saat ini: ' + currentSlug + ' (tidak berubah otomatis saat edit)';
            slugHint.className = 'text-[10px] mt-1 text-slate-500 dark:text-slate-400';
            slugHint.classList.remove('hidden');
        }

        fetch('{{ route('admin.products.check-availability', absolute: false) }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': CSRF,
            ...
            body: JSON.stringify({
                ...
                place_ids_raw: placeInput ? placeInput.value.trim() : '',
                exclude_product_id: root.dataset.excludeProductId ? parseInt(root.dataset.excludeProductId, 10) : null,
            }),
        })
        ...
        .then(function (data) {
            ...
            if (!currentSlug && slugHint) {
                ...
                } else if (data.slug_auto_suffix) {
                    slugHint.textContent = 'Slug: ' + data.slug + ' (' + data.slug_base + ' sudah dipakai — suffix otomatis)';
                    slugHint.className = 'text-[10px] mt-1 text-amber-600 dark:text-amber-400';
                    slugHint.classList.remove('hidden');
                } else if (data.slug_available) {
                    slugHint.textContent = 'Slug: ' + data.slug + ' (tersedia)';
                    slugHint.className = 'text-[10px] mt-1 text-emerald-600 dark:text-emerald-400';
                    slugHint.classList.remove('hidden');
                } else {
                    slugHint.textContent = 'Slug: ' + data.slug;
                    slugHint.className = 'text-[10px] mt-1 text-slate-500 dark:text-slate-400';
                    slugHint.classList.remove('hidden');
                }
            }
            ...
            if (!data.place_ids || data.place_ids.length === 0) {
                placeHint.textContent = 'Universal — inject di semua map (fallback jika tidak ada produk spesifik)';
                placeHint.className = 'text-[10px] mt-1 text-slate-500 dark:text-slate-400';
                placeHint.classList.remove('hidden');
                return;
            }

            if (data.place_available) {
                placeHint.textContent = 'Place ID tersedia: ' + data.place_ids.join(', ');
                placeHint.className = 'text-[10px] mt-1 text-emerald-600 dark:text-emerald-400';
            } else {
                var lines = (data.place_conflicts || []).map(function (c) {
                    return c.place_id + ' → sudah dipakai "' + c.product_name + '" (' + c.product_slug + ')';
                });
                placeHint.textContent = lines.join(' · ');
                placeHint.className = 'text-[10px] mt-1 text-red-600 dark:text-red-400';
            }

            placeHint.classList.remove('hidden');
        })
        .catch(function () {
            if (slugHint && !currentSlug) {
                slugHint.classList.add('hidden');
            }
            if (placeHint) {
...
function refreshScript(productId, productName) {
    AppPopup.warning({
        ...
        description: 'Cache dihapus dan script di-sync ulang dari GitHub ke folder lokal.',
