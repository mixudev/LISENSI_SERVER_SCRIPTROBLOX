...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        // Ngrok / reverse proxy: deteksi HTTPS dari header X-Forwarded-Proto
        if ($this->app->runningInConsole() === false) {
            $request = request();
            if ($request->header('X-Forwarded-Proto') === 'https' || str_ends_with($request->getHost(), '.ngrok-free.app') || str_ends_with($request->getHost(), '.ngrok.io')) {
                URL::forceScheme('https');
            }
        }
    }
