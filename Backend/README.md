...
Server lisensi berbasis **Laravel 13** untuk mengelola license key, validasi HWID, dan distribusi script Lua ke executor Roblox (Synapse, Solara, dll). Dilengkapi dashboard admin & user, logging API, serta proteksi akses modul script.
...
- [Struktur Script Lua](#struktur-script-lua)
...
> Ganti password segera setelah instalasi production.
...
### Opsional — GitHub script

Jika produk memakai `script_source: github`:
...
### Produk admin terpisah

Buat folder sendiri untuk script admin, misalnya `admin-tools/`, lalu set di produk:

- `access_level` = `admin`
- `script_folder` = `admin-tools`

Jangan gunakan folder yang sama untuk produk user dan admin jika ingin fitur benar-benar terpisah.
...
**Response sukses:** Lua script (Content-Type: `text/plain`)

Script response disisipi sesi modul di awal:
```lua
...
### Script tidak muncul di executor

1. Pastikan ada **produk aktif** dengan `script_folder` yang benar
2. Cek `loader.lua` ada di folder produk
3. Cek tipe lisensi cocok dengan `access_level` produk
4. Lihat log di Admin → API Logs

### Error "Sesi modul tidak valid"

- Token modul expired (6 jam) — jalankan ulang loader dari awal
- Lisensi expired/banned setelah token dibuat

### Error 500 saat aktivasi

- Pastikan migrasi terbaru sudah dijalankan (`php artisan migrate`)
...
Tunggu 1 menit atau gunakan IP berbeda. Default: 60 req/menit untuk API.
